...
export default function TimelineView({ events, tasks, routines = [], categories = [], onEditEvent, onEditTask }: TimelineViewProps) {
  ...
  const pendingRoutines = routines.filter((r) => !r.isCompleted || !r.checkedAt)
  ...
  useEffect(() => {
    ...
    gridRef.current.scrollTop = Math.max(0, scrollHour * HOUR_HEIGHT)
  }, [events])
  ...
  return (
    <div className="timeline">
      ...
      {/* 미완료 루틴 + 시간 없는 태스크 */}
      {(pendingRoutines.length > 0 || untimedTasks.length > 0) && (
        <div className="tl-untimed">
          {pendingRoutines.map((routine) => (
            <div key={routine.id} className="tl-routine-item">
              <button
                className={`tl-task-check ${routine.isCompleted ? 'done' : ''}`}
              ...
              />
              <span className={`tl-untimed-title ${routine.isCompleted ? 'tl-done' : ''}`}>{routine.title}</span>
            </div>
          ))}
          {untimedTasks.map((task) => (
            <div key={task.id} className="tl-untimed-task" onClick={() => onEditTask(task)}>
              <button
        ...
        <div className="tl-grid-inner" style={{ height: 24 * HOUR_HEIGHT }}>
          ...
          {eventGroups.map(({ event, eventCat, tasks: groupTasks }) => {
            ...
            return (
              ...
              >
                <div className="tl-event-header" onClick={() => onEditEvent(event)}>
                ...
                {groupTasks.length > 0 && (
                  <div className="tl-event-tasks">
                    {groupTasks.map((task) => {
                      ...
                      return (
                        <div key={task.id} className="tl-nested-task" onClick={() => onEditTask(task)}>
                          <button
                            className={`tl-task-check ${task.isCompleted ? 'done' : ''}`}
                            onClick={(e) => { e.stopPropagation(); toggleTaskComplete(task.id, task.isCompleted, !!task.dueDate) }}
                          >
